...
## 5.4.6 — SECURITY, VOICE CALL + STORY AUDIENCE FIXES (2026-09-13)

### Fixed
- **Voice call echo/repeat:** voice calls no longer route the same remote stream through both hidden video and audio playback elements; microphone echo cancellation, noise suppression, automatic gain control and mono capture are enabled.
- **Admin group posting:** a global admin can post to any group through the socket path, including legacy groups where membership data was incomplete.
- **Story privacy bypass:** direct story-view and highlight endpoints now enforce the same audience rules as the stories list.
- **Light-only UI:** legacy dark mode can no longer restore black screens or dark message bubbles.

### Added
- **Per-story audience:** choose accepted contacts, everyone, selected users, or only me for every new story.
- **Admin password help:** admins can reset a non-admin user's password and invalidate old sessions. Passwords remain one-way hashes; the previous password is never exposed or recoverable.

## 5.4.5 — GOLD-WHITE THEME + ALL GOLD BUTTONS (2026-09-15)

### Naya Look (v16.4.5)
- **🎯 GOLD-WHITE LIGHT THEME:** Pehle app **gold-black (dark)** thi — ab **gold-white (light)** theme default hai. Soft cream-white background (#FBF8F0), white panels, gold borders, dark brown text — aankhon ko asaani, din me use karne ke liye perfect
- **💰 SAB BUTTONS GOLD:** App ke **saare buttons ab gold gradient** me — primary, secondary, danger, ghost, send button, icon buttons, header icons, chat tabs, login tabs, admin buttons. Har button par ek hi gold family ka look (light gold → deep gold gradient + dark text)
- **🖥 APP KE SAB SCREENS UPDATE:** Login/Signup card, chat bubbles (aapke messages light-gold, dusre ke messages white), sidebar, chat header, composer, SMS popup, ads banner, toast notifications, unread badge, admin panel (login + dashboard + pills) — sab gold-white me
- **🌓 DARK MODE ABHI BHI HAI:** Purana dark look abhi bhi maujood hai — theme toggle button se **light ↔ dark switch** hota hai (dark mode gold-black hi rahega). Aap ki pasand aap ke device me save rehti hai

### Technical
- `public/style.css` — `:root` (light) + `body.dark` (dark) — dono CSS variables ka poora system
- `public/script.js` — settings avatar, profile/group/channel info avatars, badges sab gold gradient
- `server/admin/admin.html` — admin panel ka poora light gold-white theme

## 5.4.4 — Live Channel Updates + Header Fix + Story/Last-Seen Privacy + Channel DP + Admin Store (2026-09-15)

### Bug Fixes (v16.4.4 verified + fixed)
- **C1 LIVE CHANNEL POSTS:** Server `new-channel-message` event emit karta tha par client listener tha hi nahi — naye channel posts **live update nahi hote the** (reload karna padta tha). Ab full listener: open channel me message turant render, inbox preview update, aur closed channel par SMS-style popup notification
- **C2 CHANNEL POPUP CLICK:** SMS popup card pe channel notification click karne par galat user-chat khulta tha — ab sahi channel khulta hai
- **C3 SIDEBAR HEADER (Tools button):** Header me 7 buttons ek row me the — **mobile choti screen par Tools button hide/clip ho jata tha**. Ab header **2-row layout** me: Row 1 = title + Notifications + Help + Menu / Row 2 = Stories + New Group + New Channel + Tools. Sab buttons ab har screen par properly visible
- **C4 STORY PRIVACY:** Stories sab users ko dikhti thi (privacy nahi). Ab **owner ki privacy setting** ke hisaab se: default `contacts` = **sirf accepted contacts** stories dekh sakte, `everyone` = sab ko, `nobody` = koi nahi (khud + admin hamesha dekh sakta). Settings > Privacy me naya **Stories dropdown**
- **C5 LAST-SEEN PRIVITY:** `last_seen` sab ko dikhta tha. Ab `privacy_last_seen = contacts` par **sirf accepted contacts** last-seen dekh sakte, `nobody` = koi nahi (khud + admin hamesha)

### New Features (v16.4.4)
- **D1 CHANNEL DP:** Channels table me `avatar` column (auto-migration). Admin channel ki **DP change** kar sakta hai — Admin panel Channels tab "🖼 Change DP" button ya user-side Channel Info "🖼 Change DP". Sab subscribers ko **live update** (`channel-updated` socket event) — chat list, chat header, info modal sab me DP dikhega
- **D2 ADMIN CHANNEL POSTS VIEWER:** Admin panel Channels tab me **"📂 Posts"** button — channel ke **sab posts list + preview (pic/video/file)** + **individual post delete** (live: `channel-message-deleted` event)
- **D3 ADMIN GROUP MESSAGES VIEWER:** Admin panel Groups tab me **"📂 Messages"** button — group ke **sab messages + preview** + **individual message delete** (live: `message-deleted` event)
- **D4 ADMIN STORE / UPLOADS BROWSER:** Admin panel header me **"🗄 Store"** button — **sab uploaded files** (profiles, stories, wallpapers, images, videos, voice, files) tabs ke saath: **All / Pics / Videos / Files**. Har file ka **owner + size + time + kind** + inline preview + **Open** + **Delete** (file server se permanently remove)
- **D5 DB AUTO-MIGRATION:** Purani DB bhi automatically upgrade ho jayegi — `users.privacy_stories` (DEFAULT 'contacts') + `channels.avatar` columns auto-add

### Technical
- New endpoints: `POST /api/admin/channels/:id/avatar`, `GET /api/admin/channels/:id/posts`, `GET /api/admin/groups/:id/messages`, `DELETE /api/admin/channel-messages/:msgId`, `DELETE /api/admin/group-messages/:msgId`, `GET /api/admin/store?tab=`, `POST /api/admin/store/delete`
- New socket events: `channel-updated`, `channel-message-deleted`
- New helpers: `isAcceptedContact()`, `getContactMap()` — privacy filter + inbox me channel avatar expose
- `/api/privacy` ab `stories` field bhi accept karta hai; `publicUser()` privacy-aware last_seen
- APP_VERSION → 5.4.4
