/* ============================================================
   C$K4 Chat v5.4.0 — script.js (Client)
...
let unreadByChat = {};
let channelLastPost = {};        // v5.4.0: channelId -> {text, ts} (inbox se channel last post)
...
/* Premium click feedback: every button gets a tactile press + soft ripple,
   while existing action-specific toasts remain the source of truth. */
document.addEventListener('click', (event) => {
  ...
  setTimeout(() => button.classList.remove('button-clicked'), 360);
  if (!button.matches('.btn, .header-icon-btn, .icon-btn, .send-btn, .modal-close')) return;
...
async function enterApp() {
  ...
  /* v16.4.5: always use the soft gold-white product theme */
  document.body.classList.remove('dark');
  localStorage.setItem('csk4_theme', 'light');
  initEmojiPicker();
  initSocket();
  await Promise.all([loadUsers(), loadGroups(), loadChannels(), loadStories(), loadNotifications(), loadSiteSettings(), loadAdsBanner()]);
  await fetchInbox();   // v5.4.0: inbox-style previews + unread badges page-load par bhi
  initSupportHandlers();
  renderChatList();
  renderStoriesBar();
  setupComposer();
...
function initSocket() {
  ...
  /* --- channel messages (v16.4.4 FIX: live channel post updates) --- */
  socket.on('new-channel-message', (m) => {
    /* v5.4.0 inbox preview cache update */
    channelLastPost[m.channel_id] = { text: previewOf(m), ts: m.timestamp };
    if (currentChannel && m.channel_id === currentChannel.id) {
      /* avoid duplicate if already rendered (admin broadcast ack) */
      if (!channelMsgs.find(x => x.id === m.id)) channelMsgs.push(m);
      renderChannelMessages();
    } else {
      const chatKey = 'c_' + m.channel_id;
      unreadByChat[chatKey] = (unreadByChat[chatKey] || 0) + 1;
      /* SMS-style popup notification for new channel post */
      const chName = (channels.find(c => c.id === m.channel_id)?.name) || 'Channel';
      smsPopup('\ud83d\udce2 ' + chName, previewOf(m), chatKey, 'channel', m.channel_id);
    }
    renderChatList();
  });

  /* --- v16.4.4: channel DP/name live update + admin post delete --- */
  socket.on('channel-updated', (d) => {
    const ch = channels.find(x => x.id === d.channel_id);
    if (ch) { ch.avatar = d.avatar || ch.avatar; if (d.name) ch.name = d.name; }
    if (currentChannel && currentChannel.id === d.channel_id) {
      currentChannel.avatar = d.avatar || currentChannel.avatar;
      if (d.name) currentChannel.name = d.name;
      refreshChatHeaderStatus();
    }
    renderChatList();
  });
  socket.on('channel-message-deleted', (d) => {
    const idx = channelMsgs.findIndex(x => x.id === d.id);
    if (idx > -1) { channelMsgs[idx].deleted_for_everyone = 1; channelMsgs[idx].message = ''; channelMsgs[idx].media_url = null; renderChannelMessages(); }
    if (channelLastPost[d.channel_id] && channelLastPost[d.channel_id].text) {
      loadInboxData ? loadInboxData() : fetchInbox();
    }
  });
...
function notifyToast(m, isGroup = false) {
  ...
  const body = m.type === 'poll' ? '📊 Poll: ' + (m.message || '').replace('📊 POLL: ', '')
    ...
    : previewOf(m);
  /* v5.4.0: SMS-style popup card + soft ringtone — har incoming message par (chat open ho ya na ho) */
  smsPopup(name, body, isGroup ? 'g_' + m.group_id : 'u_' + m.from_user, isGroup ? 'group' : 'user', m.from_user);
...
/* ---------------- v5.4.0: SMS-STYLE POPUP CARD + RINGTONE ---------------- */
let smsPopupTimeout = null;
let smsPopupQueue = [];
function smsPopup(name, body, chatKey, kind, fromId) {
  const wrap = $('smsPopupWrap');
  if (!wrap) return;
  /* agar popup khula hai to queue me daalo (jab tak current 4s rahega) */
  if (wrap.children.length >= 3) { smsPopupQueue.push([name, body, chatKey, kind, fromId]); return; }
  _renderSmsPopup(name, body, chatKey, kind, fromId);
}
function _renderSmsPopup(name, body, chatKey, kind, fromId) {
  const wrap = $('smsPopupWrap');
  const el = document.createElement('div');
  el.className = 'sms-popup';
  const avatarInitial = esc(initialOf(name || 'U'));
  const snippet = esc((body || '').slice(0, 90));
  el.innerHTML = `
    <div class="sms-av">${avatarInitial}</div>
    <div class="sms-body">
      <div class="sms-top"><span class="sms-name">${esc(name || 'New message')}</span><span class="sms-time">now</span></div>
      <div class="sms-text">${snippet}…</div>
    </div>
    <button class="sms-close" title="Dismiss">✕</button>`;
  wrap.appendChild(el);
  requestAnimationFrame(() => el.classList.add('show'));
  playNotifySound();
  /* click → chat open karo */
  el.addEventListener('click', (e) => {
    if (e.target.classList.contains('sms-close')) { _hideSmsPopup(el); return; }
    _hideSmsPopup(el);
    if (kind === 'group') { switchTab('groups'); openGroupChat(chatKey.replace('g_', '')); }
    else if (kind === 'channel') { switchTab('channels'); openChannelChat(chatKey.replace('c_', '')); }
    else { switchTab('chats'); openChatWith(fromId); }
  });
  const autoT = setTimeout(() => _hideSmsPopup(el), 4200);
  el._autoT = autoT;
  void autoT;
}
function _hideSmsPopup(el) {
  ...
  setTimeout(() => {
    el.remove();
    const next = smsPopupQueue.shift();
    if (next) _renderSmsPopup(...next);
  }, 260);
}
...
/* v5.4.0: soft marimba-style chime — harsh beep ki jagah (harmonics + exponential decay = pleasant) */
function chime(freq, at, dur, vol) {
  const ctx = getAudioCtx();
  if (!ctx) return;
  const t0 = ctx.currentTime + (at || 0);
  [[freq, 'sine', vol], [freq * 2, 'sine', vol * 0.3], [freq * 3, 'sine', vol * 0.12]].forEach(([f, type, v]) => {
    const osc = ctx.createOscillator();
...
function playNotifySound() {
  /* v5.4.0: soft two-note rising chime (E6 → A6) — WhatsApp-style gentle "ding-dong" */
  chime(1318.5, 0.00, 0.35, 0.13);
  chime(1760.0, 0.12, 0.50, 0.11);
}

let ringtoneInterval = null;
function startRingtone() {
  if (ringtoneInterval) return;
  /* v5.4.0: modern smartphone-style ring — soft 5-note melodic pattern ×2 (harsh buzzer ki jagah) */
  const ring = () => {
    const ctx = getAudioCtx();
    if (!ctx) return;
    const notes = [880, 1108.7, 880, 1108.7, 1318.5];
    notes.forEach((f, i) => { chime(f, 0 + i * 0.22, 0.18, 0.15); chime(f, 1.5 + i * 0.22, 0.18, 0.15); });
  };
  ring();
  ringtoneInterval = setInterval(ring, 3800);
}
...
/* ---------------- v5.4.0: INBOX (last message + time + unread per chat) ---------------- */
async function fetchInbox() {
  try {
    const data = await api('/api/inbox');
    /* DMs: peer/last/unread */
    (data.dms || []).forEach(d => {
      lastMsgPreview['u_' + d.peer] = { text: d.last?.text || '', ts: d.last?.ts || 0 };
      unreadByChat['u_' + d.peer] = d.unread || 0;
      /* online status bhi inbox se refresh */
      const u = (users || []).find(x => x.id === d.peer);
      if (u) { u.online = !!d.online; u.username = d.username || u.username; }
    });
    /* Groups: group_id/last/unread/can_post */
    (data.groups || []).forEach(g => {
      lastMsgPreview['g_' + g.group_id] = g.last ? { text: g.last.text || '', ts: g.last.ts || 0 } : { text: '', ts: 0 };
      unreadByChat['g_' + g.group_id] = g.unread || 0;
      const lg = (groups || []).find(x => x.id === g.group_id);
      if (lg) { lg.can_post = g.can_post; lg.member_count = g.member_count; lg.my_role = g.my_role; lg.post_mode = g.post_mode; }
    });
    /* Channels: last post preview */
    channelLastPost = {};
    (data.channels || []).forEach(c => {
      if (c.last) channelLastPost[c.channel_id] = { text: c.last.text || '', ts: c.last.ts || 0 };
      /* v16.4.4: channel DP inbox se bhi sync */
      const lc = (channels || []).find(x => x.id === c.channel_id);
      if (lc && c.avatar) lc.avatar = c.avatar;
    });
    renderChatList();
  } catch (e) { console.error('fetchInbox', e.message); }
}
...
function renderChatList() {
  ...
  if (activeTab === 'groups') {
    ...
    filtered.forEach(g => {
      ...
      <div class="chat-item ${currentGroup && currentGroup.id === g.id ? 'active' : ''}" onclick="openGroupChat('${g.id}')">
        <div class="chat-item-img">${avatarHtml(g)}<span class="online-dot" style="background:#B8913F;color:#241C0E"></span></div>
        <div class="chat-item-info">
  ...
  if (activeTab === 'channels') {
    ...
    filtered.forEach(c => {
      const cprev = channelLastPost[c.id];   /* v5.4.0: channel last post preview (inbox) */
      html += `
      <div class="chat-item ${currentChannel && currentChannel.id === c.id ? 'active' : ''}" onclick="openChannelChat('${c.id}')">
        <div class="chat-item-img">${c.avatar ? `<img src="${esc(c.avatar)}" alt="" onerror="this.outerHTML='📢'">` : '📢'}</div>
        <div class="chat-item-info">
          <div class="chat-item-name"><span>${esc(c.name)}</span><span class="time">${cprev ? timeStr(cprev.ts) : timeStr(c.created_at)}</span></div>
          <div class="chat-item-last">
            <span class="preview">${esc((cprev?.text || c.description || 'Channel').slice(0, 40))}</span>
            ${c.subscribed ? '<span class="badge" style="background:#B8913F;color:#241C0E">Subscribed</span>' : ''}
          </div>
        </div>
      </div>`;
    });
...
async function openGroupChat(groupId) {
  ...
  /* v5.4.0: posting permission (admin-only posting default; admin Allow/Block karta hai) */
  if (currentGroup && currentGroup.can_post === undefined) {
    try {
      const inbox = await api('/api/inbox');
      const gi = (inbox.groups || []).find(x => x.group_id === groupId);
      if (gi) { currentGroup.can_post = gi.can_post; currentGroup.my_role = gi.my_role; }
    } catch {}
  }
  showChatUI('group');
}
...
function showChatUI(kind) {
  ...
  /* composer visibility — channels: only owner can post; groups: v5.4.0 admin-only posting + Allow/Block permission */
  let canPost = true;
  let blockNote = '';
  if (isChannel) {
    canPost = !!(currentChannel && currentChannel.owner_id === ME.id);
    if (!canPost) blockNote = '📢 Sirf channel owner post kar sakta hai — replies via reaction/DM';
  } else if (isGroup) {
    const isAdminRole = (currentGroup && (String(currentGroup.my_role) === 'admin')) || (ME && ME.isAdmin);
    const cp = currentGroup ? currentGroup.can_post : 1;
    canPost = isAdminRole || (cp === undefined || String(cp) === '1' || cp === null);
    if (!canPost) blockNote = '🚫 Admin ne is group me posting BLOCK ki hai — sirf padh sakte ho. Admin se Allow maango.';
  }
  const composer = $('chatUI')?.querySelector('.composer');
  if (composer) composer.style.display = canPost ? 'flex' : 'none';
  const gNote = $('groupPostNote');
  if (gNote) { gNote.style.display = (!canPost && blockNote) ? 'flex' : 'none'; if (blockNote) gNote.textContent = blockNote; }
...
function refreshChatHeaderStatus() {
  ...
  else if (currentChannel) av.innerHTML = currentChannel.avatar ? `<img src="${esc(currentChannel.avatar)}" alt="" onerror="this.outerHTML='📢'">` : '📢';
}
...
async function loadChannelMessages(channelId) {
  try {
    ...
    /* v16.4.4: channel DP sync (fresh from server) */
    if (data.channel && data.channel.avatar) {
      currentChannel.avatar = data.channel.avatar;
      const lc = channels.find(x => x.id === channelId);
      if (lc) lc.avatar = data.channel.avatar;
      refreshChatHeaderStatus();
    }
    renderChannelMessages();
  } catch (e) {
...
function viewMedia(url, kind) {
  if (kind === 'image') {
    ...
    if (w) w.document.write(`<title>C$K4 Media</title><body style="margin:0;background:#FBF8F0;display:flex;align-items:center;justify-content:center;height:100vh"><img src="${url}" style="max-width:100%;max-height:100%">`);
...
function openStoryCreator() {
  ...
  setTimeout(() => {
    ...
    $('stTags') && ($('stTags').value = '');
    $('stAudience') && ($('stAudience').value = 'contacts');
    storyAudienceChanged();
...
function storyAudienceChanged() {
  const mode = $('stAudience')?.value || 'contacts';
  const box = $('stAudienceUsers');
  if (!box) return;
  box.classList.toggle('hidden', mode !== 'selected');
  if (mode !== 'selected') return;
  box.innerHTML = users.filter(u => u.id !== ME.id).map(u => `
    <label class="user-pick story-audience-pick">
      <input type="checkbox" value="${esc(u.id)}">
      <span class="up-av">${u.profile_pic ? `<img src="${esc(u.profile_pic)}" alt="">` : esc(initialOf(u.username))}</span>
      <span style="flex:1"><b>${esc(u.username)}</b><small>${esc(u.email || '')}</small></span>
    </label>`).join('') || '<div class="small-note">Koi doosra user available nahi.</div>';
}
...
async function postStory() {
  ...
  const audience = $('stAudience')?.value || 'contacts';
  const audienceUsers = audience === 'selected'
    ? [...document.querySelectorAll('#stAudienceUsers input[type="checkbox"]:checked')].map(el => el.value)
    : [];
  ...
  try {
    ...
    if (audienceUsers.length) fd.append('audience_users', JSON.stringify(audienceUsers));
...
function renderCurrentStory() {
  ...
  if (st.type === 'text') {
    ...
    /* v5.4.0 FIX: video load/play hone tak timer NAHI chalega (network delay mein story time waste hota tha) */
    ...
    c.innerHTML = '';
    c.appendChild(vid);
    c.dataset.vidWaiting = '1';
    clearTimeout(storyViewTimer);
    /* video start hone par: normal 5s advance (chhoti videos ke liye) */
    vid.addEventListener('playing', () => {
      delete c.dataset.vidWaiting;
      if (storyViewTimer) clearTimeout(storyViewTimer);
      if (vid.duration && isFinite(vid.duration) && vid.duration > 0 && vid.duration <= 15) {
        /* chhoti video (≤15s): poori video duration ke barabar progress */
        storyViewTimer = setTimeout(() => nextStory(), Math.max(vid.duration * 1000 - (vid.currentTime * 1000), 500));
      } else {
        storyViewTimer = setTimeout(() => nextStory(), 5000);
      }
    });
    /* user ne pause kiya → timer ruk jaye */
    vid.addEventListener('pause', () => {
      if (!vid.ended) { clearTimeout(storyViewTimer); storyViewTimer = null; }
    });
    /* video khatam → agli story */
    vid.addEventListener('ended', () => {
      clearTimeout(storyViewTimer); storyViewTimer = null;
      nextStory();
    });
    /* video load fail (network) → 5s baad skip — timer abhi se nahi chal raha tha */
  ...
  /* auto-advance timer (5s — matches CSS spFill animation) — TEXT/IMAGE only.
     v5.4.0: VIDEO ke liye fixed timer NAHI — 'playing'/'ended' events se chalta hai (network delay fix) */
  if (st.type !== 'video') {
    clearTimeout(storyViewTimer);
    storyViewTimer = setTimeout(() => nextStory(), 5000);
  }
...
async function openChatInfo() {
  ...
  if (currentChatUser) {
    ...
    <div style="text-align:center;padding:8px 0 4px">
      <div style="width:92px;height:92px;border-radius:50%;background:linear-gradient(135deg,#EFCF8F,#D9B872);color:#241C0E;font-size:38px;font-weight:700;display:flex;align-items:center;justify-content:center;overflow:hidden;margin:0 auto 10px">${u.profile_pic ? `<img src="${esc(u.profile_pic)}" style="width:100%;height:100%;object-fit:cover" alt="">` : esc(initialOf(u.username))}</div>
    ...
    const amAdmin = !!(ME && (ME.isAdmin || ME.is_admin));
    inner = `
    <div style="text-align:center;padding:8px 0 4px">
      <div style="width:92px;height:92px;border-radius:50%;background:linear-gradient(135deg,#EFCF8F,#D9B872);color:#241C0E;font-size:38px;font-weight:700;display:flex;align-items:center;justify-content:center;overflow:hidden;margin:0 auto 10px">${currentChannel.avatar ? `<img src="${esc(currentChannel.avatar)}" style="width:100%;height:100%;object-fit:cover" alt="">` : '📢'}</div>
    ...
    <div style="padding:4px 16px 8px;display:flex;gap:8px;flex-wrap:wrap">
      ...
      ${amAdmin ? `<button class="btn btn-sm btn-secondary" onclick="changeChannelDp()">💼 Change DP</button>` : ''}
    </div>
    <div class="mt-16"></div>`;
  } else {
    /* my own profile */
...
async function setupPeer(remoteId, offer, role) {
  ...
  /* A voice call must have exactly one playback sink. Previously the same
     stream was assigned to hidden remoteVideo and remoteAudio, doubling audio. */
  if (rv) rv.srcObject = currentCall?.type === 'video' ? remoteStream : null;
  if (ra) {
    ra.srcObject = currentCall?.type === 'video' ? null : remoteStream;
    ra.volume = 0.9;
  }
  peerConnection.ontrack = (ev) => {
    if (!remoteStream.getTracks().some(track => track.id === ev.track.id)) remoteStream.addTrack(ev.track);
...
async function openSettings() {
  ...
  <div style="text-align:center;padding:4px 0 10px">
    <div style="width:76px;height:76px;border-radius:50%;background:linear-gradient(135deg,#EFCF8F,#D9B872);color:#241C0E;font-size:30px;font-weight:700;display:flex;align-items:center;justify-content:center;overflow:hidden;margin:0 auto 8px">${ME.profile_pic ? `<img src="${esc(ME.profile_pic)}" style="width:100%;height:100%;object-fit:cover" alt="">` : esc(initialOf(ME.username))}</div>
  ...
  <div class="settings-row" onclick="toggleTheme()">
    ...
    <span class="sr-label">Gold-white theme</span>
    <span class="sr-sub">Soft light mode enabled</span>
  </div>
  ...
  <div class="form-field"><label>Stories</label><select id="prvStories">${privOpts(p.stories || 'contacts')}</select></div>
...
function toggleTheme() {
  ...
  localStorage.setItem('csk4_theme', 'light');
  openSettings();
  toast('✨ Gold-white theme ON', 'info', 1400);
}
...
async function savePrivacy() {
  try {
    const data = await api('/api/privacy', { method: 'POST', body: {
      ...
      stories: $('prvStories') ? $('prvStories').value : undefined,
...
/* ============================================================
   v5.4.0: HELP & SUPPORT (social links + WhatsApp + email + tickets)
   ============================================================ */
const SOCIAL_META = {
  whatsapp_number: { icon: '\u{1F4F1}', name: 'WhatsApp Help Line', sub: '03347382564 — direct admin WhatsApp', prefix: 'https://wa.me/' },
  help_email:      { icon: '\u2709\uFE0F', name: 'Email Support', sub: 'cryptixshadowkernel@gmail.com', prefix: 'mailto:' },
  tiktok:          { icon: '\u{1F3AE}', name: 'TikTok', prefix: '' },
  facebook:        { icon: '\u{1F3E0}', name: 'Facebook', prefix: '' },
  github:          { icon: '\u{1F9E9}', name: 'GitHub', prefix: '' },
  x:               { icon: '\u2716\uFE0F', name: 'X (Twitter)', prefix: '' },
  linkedin:        { icon: '\u{1F4BC}', name: 'LinkedIn', prefix: '' },
  telegram:        { icon: '\u2708\uFE0F', name: 'Telegram', prefix: '' },
  telegram_channel:{ icon: '\u{1F4E3}', name: 'Telegram Channel', prefix: '' },
  whatsapp_channel:{ icon: '\u{1F4E2}', name: 'WhatsApp Channel', prefix: '' },
  instagram:       { icon: '\u{1F4F7}', name: 'Instagram', prefix: '' },
  youtube:         { icon: '\u{1F534}', name: 'YouTube', prefix: '' }
};

let siteSettings = null;
async function loadSiteSettings() {
  try { const d = await api('/api/settings'); siteSettings = d.settings || {}; } catch (e) { console.error('settings', e.message); }
  return siteSettings;
}

async function openHelpSupport() {
  if (!siteSettings) await loadSiteSettings();
  const st = siteSettings || {};
  const wa = (st.whatsapp_number || '03347382564').replace(/[^0-9+]/g, '');
  const email = st.help_email || 'cryptixshadowkernel@gmail.com';
  const rows = Object.entries(SOCIAL_META).map(([k, meta]) => {
    const val = st[k] || '';
    if (!val || k === 'whatsapp_number' || k === 'help_email') return '';
    return `<a class="help-social-row" href="${esc(val)}" target="_blank" rel="noopener">
      <span class="hs-ico">${meta.icon}</span>
      <span class="hs-body"><span class="hs-name">${esc(meta.name)}</span>
      <span class="hs-sub">${esc(val)}</span></span>
      <span style="color:var(--accent)">\u25B8</span>
    </a>`;
  }).join('');
  $('helpSupportBody').innerHTML = `
    <div class="divider-label">\u{1F4DE} Direct Help Line</div>
    <div class="help-contact-card">
      <a href="https://wa.me/${esc(wa.startsWith('92') ? wa : '92' + wa.replace(/^0+/, ''))}" target="_blank" rel="noopener">\u{1F4F1} WhatsApp</a>
      <a class="mail" href="mailto:${esc(email)}">\u2709\uFE0F Email</a>
    </div>
    <div class="divider-label">\u{1F517} Social Media</div>
    ${rows || '<div class="small-note" style="padding:8px 0">Links admin panel se update hoti hain</div>'}
    <div class="divider-label">\u{1F4E9} Message to Admin (Help Chat)</div>
    <div class="form-field"><label>Type</label>
      <select id="spType">
        <option value="problem">\u26A0\uFE0F Problem / Bug</option>
        <option value="feedback">\u{1F44D} Feedback</option>
        <option value="suggestion">\u{1F4A1} Suggestion</option>
        <option value="other">\u{1F4AC} Other</option>
      </select>
    </div>
    <div class="form-field"><label>Subject</label><input id="spSubject" maxlength="120" placeholder="Short subject"></div>
    <div class="form-field"><label>Message</label><textarea id="spMessage" rows="4" maxlength="2000" placeholder="Admin ko message, review ya suggestion likho\u2026"></textarea></div>
    <button class="btn btn-full" onclick="sendSupportTicket()">\u{1F4E8} Send to Admin</button>
    <div style="margin-top:14px">
      <div class="divider-label">\u{1F4DC} My Messages & Admin Replies</div>
      <div id="myTickets"><div class="small-note" style="padding:8px 0">Loading\u2026</div></div>
    </div>`;
...
async function sendSupportTicket() {
  const type = $('spType')?.value || 'other';
  const subject = ($('spSubject')?.value || '').trim();
  const message = ($('spMessage')?.value || '').trim();
  if (!message) return toast('Message likho \u26A0\uFE0F', 'error');
  try {
    const d = await api('/api/support', { method: 'POST', body: { type, subject, message } });
    toast(d.message || '\u2705 Sent', 'success');
    if ($('spSubject')) $('spSubject').value = '';
    if ($('spMessage')) $('spMessage').value = '';
    loadMyTickets();
  } catch (e) { toast(e.message, 'error'); }
}
...
async function loadMyTickets() {
  ...
  if (!box) return;
  try {
    const d = await api('/api/support/my');
    const list = d.tickets || [];
    box.innerHTML = list.length ? list.map(t => renderTicket(t, false)).join('')
      : '<div class="small-note" style="padding:8px 0">Abhi tak koi message nahi bheja</div>';
  } catch (e) { box.innerHTML = '<div class="small-note">\u26A0\uFE0F ' + esc(e.message) + '</div>'; }
}

function renderTicket(t, isAdminView) {
  const status = String(t.status || 'open');
  const reply = t.admin_reply ? `<div class="tk-reply">\u{1F4DD} <b>Admin:</b> ${esc(t.admin_reply)}</div>` : '';
  return `<div class="ticket-item">
    <div class="tk-type">${esc(t.type || 'other')} ${isAdminView && t.username ? '• ' + esc(t.username) : ''}</div>
    ${t.subject ? `<div class="tk-subject">${esc(t.subject)}</div>` : ''}
    <div class="tk-msg">${esc(t.message || '')}</div>
    ${reply}
    <div class="tk-meta"><span class="tk-status ${status}">${status.toUpperCase()}</span> <span>${timeStr(t.created_at)}</span>${t.replied_at ? ' <span>• replied ' + timeStr(t.replied_at) + '</span>' : ''}</div>
    ${isAdminView ? `
    <div style="display:flex;gap:6px;margin-top:8px">
      <button class="btn-sm" style="background:var(--accent);color:#201a0d;border:none;font-weight:700;padding:6px 12px;border-radius:8px;cursor:pointer" onclick="replySupportTicket('${t.id}')">\u{1F4DD} Reply</button>
      ${status !== 'closed' ? `<button class="btn-sm" style="background:transparent;border:1px solid var(--border);color:var(--text);padding:6px 12px;border-radius:8px;cursor:pointer" onclick="closeSupportTicket('${t.id}')">\u2715 Close</button>` : ''}
    </div>` : ''}
  </div>`;
}
...
async function openToolsModal() {
  ...
  try {
    ...
    box.innerHTML = tools.length ? tools.map(t => `
      <a class="help-social-row" href="${esc(t.url)}" target="_blank" rel="noopener">
        <span class="hs-ico">${t.icon || '\u{1F6E0}\uFE0F'}</span>
        <span class="hs-body"><span class="hs-name">${esc(t.name)}</span>
        <span class="hs-sub">${esc(t.description || t.url)}</span></span>
        <span style="color:var(--accent)">\u25B8</span>
      </a>`).join('') + '<div class="small-note" style="padding:6px 0 2px">Tools admin panel se manage hote hain</div>'
      : '<div class="empty-state" style="padding:40px 20px"><div class="es-ico">\u{1F6E0}\uFE0F</div><div class="small-note">Koi tool available nahi</div></div>';
  } catch (e) { box.innerHTML = '<div class="small-note">\u26A0\uFE0F ' + esc(e.message) + '</div>'; }
...
function renderAdsBanner() {
  const wrap = $('adsBannerWrap');
  if (!wrap) return;
  if (adsDismissed || !activeAds.length) { wrap.innerHTML = ''; return; }
  const ad = activeAds[0];
  wrap.innerHTML = `
    <div class="ads-banner">
      <button class="ad-close" onclick="dismissAd()">\u2715</button>
      <span class="ad-ico">\u{1F514}</span>
      <div class="ad-body">
        <div class="ad-title">${esc(ad.title)}</div>
        <div class="ad-text">${esc(ad.body || '')}</div>
      </div>
      ${ad.link_url ? `<a class="ad-cta" href="${esc(ad.link_url)}" target="_blank" rel="noopener">${esc(ad.link_label || 'Open')}</a>` : ''}
    </div>`;
}
function dismissAd() { adsDismissed = true; renderAdsBanner(); }

/* ---------------- v5.4.0: SUPPORT SOCKET HANDLERS ---------------- */
function initSupportHandlers() {
  if (!socket) return;
  socket.on('support-reply', (d) => {
    toast(`\u{1F4DD} Admin ne reply kiya: ${String(d.reply || '').slice(0, 60)}`, 'success', 4200);
    if ($('helpSupportModal')?.classList.contains('open')) loadMyTickets();
  });
}

/* ---------------- v16.4.4: CHANNEL DP CHANGE (admin) ---------------- */
let chDpFile = null;
function changeChannelDp() {
  if (!currentChannel) return;
  const amAdmin = !!(ME && (ME.isAdmin || ME.is_admin));
  if (!amAdmin) { toast('Sirf Admin channel DP change kar sakta hai', 'error'); return; }
  const input = document.createElement('input');
  input.type = 'file'; input.accept = 'image/*';
  input.onchange = async () => {
    chDpFile = input.files && input.files[0];
    if (!chDpFile) return;
    const fd = new FormData();
    fd.append('avatar', chDpFile);
    try {
      const data = await api('/api/admin/channels/' + currentChannel.id + '/avatar', { method: 'POST', form: fd });
      currentChannel.avatar = data.avatar;
      const lc = channels.find(x => x.id === currentChannel.id);
      if (lc) lc.avatar = data.avatar;
      refreshChatHeaderStatus();
      renderChatList();
      toast(data.message || 'Channel DP updated 💼', 'success');
    } catch (e) { toast(e.message, 'error'); }
  };
  input.click();
}
