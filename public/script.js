  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (data && Array.isArray(data.details) && data.details.length) {
        msg = data.details.map(d => d.msg || d.message || '').filter(Boolean).join(' • ') || msg;
      }
    ...
    if (String(e.message).includes('Invalid or expired token') || String(e.message).includes('No token')) {
...
function switchAuthTab(tab) {
  ...
  const wrap = $('regUsernameField');
  if (wrap) wrap.classList.toggle('hidden', isLogin);
  const pwHint = $('pwHint');
...
async function handleAuth() {
  ...
  if (!isLogin) {
    if (username.length < 2) { errEl.textContent = 'Username kam se kam 2 characters ka likho!'; return; }
    if (username.length > 30) { errEl.textContent = 'Username 30 characters se zyada nahi ho sakta!'; return; }
    if (password.length < 8) { errEl.textContent = 'Password kam se kam 8 characters ka hona chahiye!'; return; }
    if (!/[A-Z]/.test(password)) { errEl.textContent = 'Password me ek badda (uppercase) letter hona chahiye!'; return; }
    if (!/[a-z]/.test(password)) { errEl.textContent = 'Password me ek chhota (lowercase) letter hona chahiye!'; return; }
    if (!/[0-9]/.test(password)) { errEl.textContent = 'Password me ek number hona chahiye!'; return; }
  }
...
function renderChatList() {
  ...
  if (activeTab === 'chats') {
    const chatUsers = users.filter(u => u.id !== ME.id);
    const filtered = q ? chatUsers.filter(u => (u.username || '').toLowerCase().includes(q)) : chatUsers;
    filtered.sort((a, b) => (lastMsgPreview['u_' + b.id]?.ts || 0) - (lastMsgPreview['u_' + a.id]?.ts || 0));
    if (!filtered.length) html = `<div class="empty-state" style="padding:40px 20px"><div class="es-ico">👤</div><div class="small-note">${q ? 'Koi user nahi mila' : 'Register karo dusre accounts se, phir yahan dikhenge!'}</div></div>`;
    filtered.forEach(u => {
      const key = 'u_' + u.id;
      ...
      <div class="chat-item ${currentChatUser && currentChatUser.id === u.id ? 'active' : ''}" onclick="openChatWith('${u.id}')">
        ...
        <div class="chat-item-info">
          <div class="chat-item-name"><span>${esc(u.username)} ${u.is_admin ? '👑' : ''} ${u.email === 'bot@csk4.com' ? '🤖' : ''}</span><span class="time">${prev ? timeStr(prev.ts) : ''}</span></div>
          <div class="chat-item-last">
            <span class="preview">${esc((prev?.text || u.bio || 'Hey there! I am using C$K4 Chat! 🚀').slice(0, 40))}</span>
  ...
  if (activeTab === 'groups') {
    ...
    if (!filtered.length) html = `<div class="empty-state" style="padding:40px 20px"><div class="es-ico">👥</div><div class="small-note">Koi group nahi — header me 👥 se banao!</div></div>`;
    filtered.forEach(g => {
      const key = 'g_' + g.id;
      const prev = lastMsgPreview[key];
      const unread = unreadByChat[key] || 0;
  ...
  if (activeTab === 'channels') {
    ...
    if (!filtered.length) html = `<div class="empty-state" style="padding:40px 20px"><div class="es-ico">📢</div><div class="small-note">Koi channel nahi — header me 📢 se banao!</div></div>`;
    filtered.forEach(c => {
      html += `
...
function renderMessageBubble(m, side, isGroup, isChannel = false) {
  ...
  const actions = msgActionsHtml(m, mine, isChannel);
...
function sendTyping(isTyping) {
  if (!socket) return;
  const now = Date.now();
  if (isTyping && now - typingSentAt < 1200) return;
  typingSentAt = now;
...
async function startVoiceRecording() {
  try {
    ...
    recTimerInterval = setInterval(() => {
      ...
      if (sec >= 300) stopVoiceRecording(true);
    }, 1000);
...
async function voiceToText(url) {
  ...
  try {
    const rec = new SR();
    rec.lang = 'ur-PK';
    rec.onresult = (e) => { toast('🗣️ ' + e.results[0][0].transcript, 'info', 5000); };
...
function renderCurrentStory() {
  ...
  if (!isOwner) {
    api('/api/story/view/' + st.id, { method: 'POST', body: {} }).then(() => { st.viewed = true; }).catch(() => {});
  }
...
function renderNotifList() {
  ...
  if (!body) return;
  const icons = { message: '💬', group: '👥', story: '🖼️', 'story-tag': '🏷️', call: '📞', game: '🎮', poll: '📊', report: '🚫', system: '⚙️' };
  body.innerHTML = notifications.length
    ? notifications.map(n => `
      <div class="notif-item ${n.read ? '' : 'unread'}" onclick="readOneNotif('${n.id}')">
        ...
        <div style="flex:1"><div class="ni-title">${esc(n.title)}</div><div class="ni-body">${esc(n.body)}</div></div>
...
(function () {
  ...
  console.log('[C$K4 v5.2] Mobile enhancements active');
})();
  ...
  containers.forEach(id => {
    ...
    el.addEventListener('touchend', async (e) => {
      ...
      if (diff > REFRESH_THRESHOLD && el.scrollTop <= 0) {
        ...
        try { await doSmartRefresh(id); } catch {}
